...
/*==============================================================================
  ...
  PS2 file I/O routines

==============================================================================*/

/*

        This is specific version for the PS2. Since fgetc() is a nightmare once
        using through the network, this version first load enterely the file in
        a buffer and then work directly on that buffer. This greatly increase
        loading time !
...
static UBYTE *fbuf    = NULL;
static UBYTE *fptr    = NULL;
static SLONG  foffset = 0;
static SLONG  fsize   = 0;
...
static BOOL _mm_FileReader_Read(MREADER* reader,void* ptr,size_t size)
{
	...
	if ((foffset + size ) > fsize)
	   maxsize = fsize - foffset + 1;
	if (maxsize > 0)
 	   memcpy(ptr, fptr + foffset, maxsize);
